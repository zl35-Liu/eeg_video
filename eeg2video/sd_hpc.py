...
from tuneavideo.data.dataset import TuneAVideoDataset, TuneMultiVideoDataset,TuneMultiVideoDataset1  # 自定义数据集类
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    # 加载和准备训练数据集
    train_dataset = TuneMultiVideoDataset1(**train_data)

    # 手动设置视频路径和文本提示（示例代码中可能应通过配置文件处理）
    video_path2 = './data/SEED-DV/Video'

    train_dataset.video_path = video_path2

    video_text2 = './data/SEED-DV/Video/BLIP-caption/all.txt'
    ...
    for epoch in tqdm(range(first_epoch, num_train_epochs + 1)):
        ...
        for step, batch in enumerate(train_dataloader):   #total对应multi导入的list 其中再包含若干个example
            ...
            with accelerator.accumulate(unet):  # 梯度累积上下文
                # 将视频像素值转换为潜在表示
                pixel_values = batch[step]["pixel_values"].to(weight_dtype)
                video = pixel_values/2 + 0.5
                video = video.squeeze(0)
                # print("video sahpe ",video.shape)
                prompts = batch[step]["prompt_ids"]

                input_path = f"{output_dir}/input_samples"
                if not os.path.exists(input_path):
                    os.makedirs(input_path, exist_ok=True)
                save_video_frames(video, f"{input_path}/epoch{epoch}step{step}_in.gif") # 检查输入视频质量是否正确
                ...
                # 获取文本编码
                encoder_hidden_states = text_encoder(batch[step]["prompt_ids"])[0]   #
                ...
                print(f"epoch {epoch} Processing step {step} loss {loss.item()}")
        ...
        #
        print(epoch, "step_loss", loss.detach().item(), "lr", lr_scheduler.get_last_lr()[0])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./all_40_video_hpc.yaml")
